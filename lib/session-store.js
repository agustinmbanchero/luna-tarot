...
const memoriaPerfiles = new Map();    // fallback sin Redis: numero → perfil
...
// ── Perfil del cliente: identidad persistente (vive más que la sesión) ───────
// Guarda nombre / fecha de nacimiento por número, con TTL de 1 año. Independiente de la
// sesión conversacional (24h). Permite reconocer a clientas que vuelven días/semanas después.
const TTL_PERFIL = 60 * 60 * 24 * 365; // 1 año

async function getPerfilCliente(numero) {
  const redis = getRedis();
  if (redis) {
    try {
      ...
      console.error('Redis getPerfilCliente error, usando memoria:', err.message);
      return memoriaPerfiles.get(numero) || null;
    }
  }
  return memoriaPerfiles.get(numero) || null;
}

async function guardarPerfilCliente(numero, datos) {
  // Merge con el perfil existente: nunca pisamos un dato bueno con null
  const actual = (await getPerfilCliente(numero)) || {};
  const merged = { ...actual };
  for (const k of ['nombre', 'nombreCompleto', 'fechaNacimiento']) {
    if (datos[k]) merged[k] = datos[k];
  }
  merged.ultimaVez = Date.now();

  const redis = getRedis();
  if (redis) {
    try {
      await redis.set(`cliente:${numero}`, JSON.stringify(merged), 'EX', TTL_PERFIL);
    } catch (err) {
      console.error('Redis guardarPerfilCliente error, guardando en memoria:', err.message);
      memoriaPerfiles.set(numero, merged);
    }
  } else {
    memoriaPerfiles.set(numero, merged);
  }
...
module.exports = { getSession, saveSession, deleteSession, getPendingLunaSessions, getPreguntasRestantes, setPreguntasRestantes, decrementarPregunta, marcarMensajeProcesado, adquirirLock, liberarLock, getPerfilCliente, guardarPerfilCliente };
